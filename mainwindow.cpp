...
{
    static int remainingTimeInSeconds = 5 * 60;  // 5 minutes
    ...
    for (int i = 0; i < 15; ++i)
    {
        for (int j = 0; j < 15; ++j)
        {
            ...
            else if (gameMap[i][j] == 'B')
            {
                ...
                {
                    if(std::rand() % 5 == 0) {
                        PowerUp *powerUp = new PowerUp("bomb");
                        powerUp->setPos(j * 50, i * 50);
                        scene->addItem(powerUp);
                    }
                    else {
                        PowerUp *powerUp = new PowerUp("explosion");
                        powerUp->setPos(j * 50, i * 50);
                        scene->addItem(powerUp);
                    }
                }
...
void MainWindow::checkPowerUpCollision(Player *player)
{
    // Calculate the player's position
    qreal x = player->x();
    qreal y = player->y();

    // Get a list of items at the player's position
    QList<QGraphicsItem *> items = scene->items(QRectF(x, y, player->boundingRect().width(), player->boundingRect().height()), Qt::IntersectsItemBoundingRect);
    ...
    for (QGraphicsItem *item : items)
    {
        ...
        if (powerUp)
        {
            ...
            if (powerUp->whichFactor() == "bomb")
            {
                // Activate bomb power-up for the player
                player->activateBombPowerUp();
            }
            else if (powerUp->whichFactor() == "explosion")
            {
                // Activate explosion power-up for the player
                player->activateExplosionPowerUp();
            }
